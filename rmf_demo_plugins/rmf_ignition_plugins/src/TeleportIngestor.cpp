  ...
  Entity& payload_entity)
{
  ...
  const auto robot_model_pos = ecm.Component<components::Pose>(robot_entity)->Data().Pos();
...
void TeleportIngestorPlugin::send_ingested_item_home(EntityComponentManager& ecm)
{
  ...
  {
    const auto it = _ingestor_common->_non_static_models_init_poses.find(Model(_ingested_entity).Name(ecm));
    ...
    _ingestor_common->_ingestor_filled = false;
  }
}
  ...
  EntityComponentManager& ecm, EventManager&)
{
  ...
  ignwarn << "Initializing plugin with name " << _ingestor_common->_guid << std::endl;
  _ros_node = std::make_shared<rclcpp::Node>(_ingestor_common->_guid);
  _ingestor_common->init_ros_node(_ros_node);
  RCLCPP_INFO(_ingestor_common->_ros_node->get_logger(), "Started node...");
  ...
  ecm.Each<components::Model, components::Name, components::Pose, components::Static>(
    ...
    ) -> bool
    {
      if (!is_static->Data() && name->Data() != _ingestor_common->_guid)
      {
        _ingestor_common->_non_static_models_init_poses[name->Data()] = pose->Data();
      }
      return true;
    });
...
{
  _ingestor_common->_sim_time = (std::chrono::duration_cast<std::chrono::seconds>(info.simTime).count());

  // TODO parallel thread executor?
  rclcpp::spin_some(_ingestor_common->_ros_node);
  if (!_load_complete) {
    return;
  }
  ...
  if (_ingestor_common->_sim_time - _ingestor_common->_last_pub_time >= 2.0)
  {
    _ingestor_common->_last_pub_time = _ingestor_common->_sim_time;
  ...
  if(_ingestor_common->_sim_time - _ingestor_common->_last_ingested_time >= 5.0 && _ingestor_common->_ingestor_filled){
